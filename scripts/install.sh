...
usage() {
  ...
  printf '  npm CLIs: %s, %s\n' "$PASEO_PACKAGE" "$OPENSPEC_PACKAGE"
  printf '  Python package: lane%s\n' ' (editable)'
}
...
install_system_tools() {
  ...
  if have apt-get; then
    ...
    # shellcheck disable=SC2086
    $sudo_cmd apt-get install -y git gh glab just npm python3 python3-pip
    return 0
  fi
  ...
  if have dnf; then
    sudo_cmd="$(sudo_prefix)"
    # shellcheck disable=SC2086
    $sudo_cmd dnf install -y git gh glab just npm python3 python3-pip
    return 0
  fi
  ...
  if have pacman; then
    sudo_cmd="$(sudo_prefix)"
    # shellcheck disable=SC2086
    $sudo_cmd pacman -Sy --needed --noconfirm git github-cli glab just npm python python-pip
    return 0
  fi
...
install_node_clis() {
  ...
  npm install
  npm install -g "$PASEO_PACKAGE" "$OPENSPEC_PACKAGE"
}
...
report_versions() {
  printf '\nInstalled versions:\n'
  for cmd in git gh glab just npm python3 paseo openspec; do
    if have "$cmd"; then
      printf '  %s: ' "$cmd"
