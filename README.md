...
After those gates pass, it writes `.lane/archive/<lane-id>.json` in the invoking
checkout with the lane id, branch, PR URL, merge status, archived spec id,
source path, archive status, and removed Paseo agents. The summary is first
written with archive status `pending`, then updated to `archived` after Paseo
archive succeeds; a failed archive leaves the pending summary behind as a local
diagnostic receipt. Remote branch deletion requires `--delete-remote-branch` and
a merged PR.
